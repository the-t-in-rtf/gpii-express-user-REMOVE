...
(function () {
    ...
    fluid.registerNamespace("gpii.express.couchuser.frontend.canHandleStrings");

    gpii.express.couchuser.frontend.canHandleStrings.stringToJSON = function (that, data) {
    ...
    gpii.express.couchuser.frontend.canHandleStrings.canHandleErrorString = function (that, data) {
        var safeData = data.responseJSON ? data : { responseJSON: that.stringToJSON(data.responseText)};
        that.handleError(safeData);
    };

    gpii.express.couchuser.frontend.canHandleStrings.canHandleSuccessString = function (that, data) {
        var safeData = data.responseJSON ? data : { responseJSON: that.stringToJSON(data.responseText)};
        // The data is expected to be the third argument.
        that.handleSuccess(null, null, safeData);
    };

    fluid.defaults("gpii.express.couchuser.frontend.canHandleStrings", {
        ajaxOptions: {
        ...
        invokers: {
            stringToJSON: {
                funcName: "gpii.express.couchuser.frontend.canHandleStrings.stringToJSON",
                args: ["{that}", "{arguments}.0"]
            },
            canHandleSuccessString: {
                funcName: "gpii.express.couchuser.frontend.canHandleStrings.canHandleSuccessString",
                args:     ["{that}", "{arguments}.2"]
            },
            canHandleErrorString: {
                funcName: "gpii.express.couchuser.frontend.canHandleStrings.canHandleErrorString",
                args:     ["{that}", "{arguments}.0"]
            }
